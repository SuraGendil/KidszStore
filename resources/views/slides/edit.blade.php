...
<div class="container mx-auto p-8 max-w-2xl">
    ...
    <div class="bg-[#1A255B] p-8 rounded-lg shadow-xl border border-gray-700">
        <h1 class="text-3xl font-bold text-white mb-6">Edit Slide</h1>
        ...
        <form action="{{ route('admin.slides.update', $slide->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $slide->title) }}" class="shadow appearance-none border border-gray-700 rounded w-full py-2 px-3 text-white leading-tight focus:outline-none focus:shadow-outline bg-[#0e1a4b]" required>
            </div>
            <div class="mb-4">
                <label for="image" class="block text-gray-300 text-sm font-bold mb-2">Gambar Slide Saat Ini</label>
                @if ($slide->image)
                    <img src="{{ asset('storage/' . $slide->image) }}" alt="{{ $slide->title }}" class="mb-2 max-h-40 rounded-lg">
                @else
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="4" class="shadow appearance-none border border-gray-700 rounded w-full py-2 px-3 text-white leading-tight focus:outline-none focus:shadow-outline bg-[#0e1a4b]">{{ old('description', $slide->description) }}</textarea>
            </div>
            <div class="mb-6">
                ...
                <select name="status" id="status" class="shadow appearance-none border border-gray-700 rounded w-full py-2 px-3 text-white leading-tight focus:outline-none focus:shadow-outline bg-[#0e1a4b]">
                    ...
                    <option value="0" {{ old('status', $slide->status) == 0 ? 'selected' : '' }}>Non-aktif</option>
                </select>
            </div>
