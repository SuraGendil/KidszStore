    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div class="max-w-6xl mx-auto bg-[#0e1a4b] md:rounded-3xl rounded-xl shadow-2xl overflow-hidden">
            <div class="relative w-full h-64 md:h-96 overflow-hidden">
                <img src="{{ $product->image_url ?? 'https://placehold.co/1200x400/1A255B/ffffff?text=Produk' }}"
                    alt="Gambar header untuk {{ $product->title }}" class="w-full h-full object-cover">
                <div
                    class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/50 to-transparent flex flex-col justify-end p-6 md:p-8 text-white">
                    <span class="text-lg font-semibold text-blue-300">{{ $product->game->name ?? 'Game' }}</span>
                    <h1 class="text-3xl md:text-5xl font-extrabold leading-tight drop-shadow-lg">{{ $product->title }}
                    </h1>
                    <h2 class="text-xl md:text-2xl font-bold text-gray-300">{{ $product->category->name ?? 'Kategori' }}
                    </h2>
                </div>
            </div>
            ...
            <div class="bg-[#1a2c6d] text-sm md:text-base">
                <div class="flex justify-center overflow-x-auto whitespace-nowrap p-4 space-x-6">
                    ...
                    <div class="flex items-center space-x-2 text-blue-300 transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                            <path
                                d="M10 2a5 5 0 00-5 5v2a2 2 0 00-2 2v5a2 2 0 002 2h10a2 2 0 002-2v-5a2 2 0 00-2-2H7V7a3 3 0 015.905-.75l.094.026z" />
                        </svg>
                    ...
                    <div class="flex items-center space-x-2 text-blue-300 transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                            <path
                                d="M4 4a2 2 0 00-2 2v6a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2H4zm12 4v2a2 2 0 01-2 2H6a2 2 0 01-2-2V8h12zM6 8V6h12v2H6z" />
                        </svg>
                    ...
                    <div class="flex items-center space-x-2 text-blue-300 transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd"
                                d="M10 18a8 8 0 100-16 8 8 0 000 16zm-1-4a1 1 0 102 0V8a1 1 0 10-2 0v6zm-1.5-6a1 1 0 112 0h-2z"
                                clip-rule="evenodd" />
                        </svg>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 p-6 md:p-8 text-white gap-8">
                <div class="md:col-span-2 space-y-6">
                    <div>
                        ...
                        <p class="text-gray-300 leading-relaxed">
                            Dapatkan <strong class="text-white">{{ $product->title }}</strong> untuk game <strong
                                class="text-white">{{ $product->game->name ?? '' }}</strong> dengan cepat dan aman hanya
                            di KIDSZSTORE. Ikuti langkah-langkah di bawah untuk menyelesaikan pembelian Anda.
                        </p>
                    </div>
                    <div>
                        <h3 class="text-xl font-bold text-white mb-3">Cara Transaksi</h3>
                        <ol class="list-decimal list-inside text-gray-300 space-y-2">
                            <li>Pastikan Anda sudah <strong class="text-gray-300">Login</strong> ke akun KIDSZSTORE Anda.</li>
                            <li>Pilih jumlah produk yang ingin Anda beli di sebelah kanan.</li>
                            <li>Klik tombol <strong class="text-gray-300">"Beli Sekarang"</strong> untuk melanjutkan ke halaman pembayaran.</li>
                            <li>Selesaikan pembayaran sesuai metode yang Anda pilih.</li>
                            <li class="font-semibold text-white">Setelah bayar, wajib lakukan <strong class="text-yellow-400">konfirmasi via WhatsApp</strong> dengan menyertakan <strong class="text-gray-300">ID Pesanan</strong> dan <strong class="text-gray-300">data game Anda</strong> (User ID/Nickname).</li>
                ...
                <div class="md:col-span-1">
                    <div x-data="{
                        ...
                        // fungsi handling proses payment
                        async processPayment() {
                            if (this.stock <= 0) return;
                            this.isLoading = true;
                            const component = this; // Capture Alpine's 'this' context
                            ...
                            try {
                                ...
                                window.snap.pay(data.snap_token, {
                                    onSuccess: function(result) {
                                        // LANGKAH DEBUG: Tampilkan data lengkap dari server di console browser
                                        console.log('Data lengkap dari server:', data);

                                        // Populate success data from backend response
                                        component.successData.order_id = data.order.order_id;
                                        component.successData.product_title = data.order.product_title;
                                        component.successData.total_price = data.order.total_price;
                                        component.successData.whatsapp_phone = data.whatsapp.phone;
                                        component.successData.whatsapp_message = data.whatsapp.message;

                                        // Change state to show success message
                                    ...
                                    onClose: function() {
                                        // Reset loading state if user closes the popup
                                        component.isLoading = false;
                                    }
                                });
                        ...
                        <template x-if="!paymentSuccess">
                            <div class="p-6 rounded-xl bg-[#1a2c6d] border border-blue-800 shadow-lg">
                                ...
                                @auth
                                    <button type="button" @click="processPayment()" :disabled="stock <= 0 || isLoading" class="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-6 rounded-lg text-center text-lg transition-all duration-300 flex items-center justify-center space-x-2 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed">
                                        <span x-show="!isLoading">Pesan Sekarang</span>
                        ...
                        <template x-if="paymentSuccess">
                            <div class="p-6 rounded-xl bg-[#1a2c6d] border border-green-500 shadow-lg">
                                ...
                                <p class="text-gray-300 mb-4">Terima kasih. Pesanan Anda sedang kami proses. Berikut adalah detail transaksi Anda:</p>
                                
                                <div class="space-y-2 text-white border-t border-b border-blue-800 py-4 my-4">
                                    <p><strong>ID Pesanan:</strong> <span x-text="successData.order_id"></span></p>
                                    <p><strong>Produk:</strong> <span x-text="successData.product_title"></span></p>
                                    <p><strong>Total Bayar:</strong> <span x-text="'Rp ' + new Intl.NumberFormat('id-ID').format(successData.total_price)"></span></p>
                                </div>
                
                                <a :href="`https://wa.me/${successData.whatsapp_phone}?text=${encodeURIComponent(successData.whatsapp_message)}`"
                                target="_blank"
                                class="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-6 rounded-lg text-center text-lg transition-all duration-300 flex items-center justify-center space-x-2 transform hover:scale-105">
                                    <i class="fa-brands fa-whatsapp"></i>
                                    <span>Konfirmasi via WhatsApp</span>
                                </a>
                                <a href="{{ route('dashboard') }}" class="block text-center mt-4 text-blue-300 hover:text-white">Kembali ke Dashboard</a>
                            </div>
                        </template>
                    </div>
                </div>
